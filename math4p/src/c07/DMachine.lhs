...
Suddenly, we have constant list.
...
This is just for clearness.
Polynomials are usually defined over a field,
...
The first element in the sequence is just the last coefficient,
the one ``without'' an $x$, since the other terms ``disappear'',
when we apply 0. Consider for example a polynomial of the form
$x^2 + x + a$, we get $0^2 + 0 + a = c$, where $c$ is the first
...
The local function |go| repeats from 0 to $d$,
...
We use |genCoeff| to create a matrix representing
...
generated by $x^2$, |findGen| yields:
|[0 % 1,0 % 1,0 % 1,0 % 1,1 % 1]|,
which corresponds to the polynomial $x^4$. 
...
|let ds' = tail ds in findGen (tail ds') (head ds')| and get
|[286 % 1,384 % 1,144 % 1]|,
which corresponds to the polynomial 
$144x^2 + 384x + 286$.
...
\ignore{
- guess the polynomial generating the 1./2./.. difference
- derivative
}
