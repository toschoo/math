...
are actually numbers in Haskell and are of type class |Eq|.
...
passed to |go| and, second, the result.
...
We first |weigh| them by zipping the list of coefficients with integers
...
would be created with our function as |poly [1,1,1]|.
The polynomial $5x^5 + 4x^4 + 3x^3 + 2x^2 + x$ would be
|poly [0,1,2,3,4,5]|. The polynomial
$5x^4 + 4x^3 + 3x^2 + 2x + 1$, on the other hand, 
would be created by |poly [1,2,3,4,5]|.
