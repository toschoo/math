...
To find the squared factors in an infinite field, 
the following algorithm, called after his inventor
\term{Yun's} algorithm, can be adopted.
...
the sum of the factor products.
...
we can obtain $a$ by $a$, each $a$ being the product
of all factors of the same exponent.
...
\ignore{
start from this. This is much better than the half-correct
product rule above. Note the striking similarity to the 
